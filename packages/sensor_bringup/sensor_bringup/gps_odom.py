...
class NavSatFixToOdom(Node):
    ...
    def __init__(self):
        ...
        self.lat_origin = self.get_parameter('origin.latitude').value
        self.lon_origin = self.get_parameter('origin.longitude').value
        self.alt_origin = self.get_parameter('origin.altitude').value

        self.orientation = None  # To store the latest IMU orientation

        # Subscribe to NavSatFix
        self.fix_sub = self.create_subscription(NavSatFix, 'fix', self.gps_callback, 10)
        self.imu_sub = self.create_subscription(Imu, 'imu/data', self.update_orientation, 10)
    ...
    def update_orientation(self, msg: Imu):
        '''
        Gets the orientation from the IMU message and updates the class variable.
        '''
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        # Filter out bad readings based on the number of satellites (if available)
        if msg.latitude < 0.1:
            self.get_logger().warn(f"Bad GPS status, skipping this GPS reading.", throttle_duration_sec=10)
            return
        ...
        # Convert latitude/longitude to local Cartesian coordinates
        x, y = self.CalculateHaversine(self.lat_origin,
                                       self.lon_origin,
                                       msg.latitude,
                                       msg.longitude)
        
        # Access the altitude (z) value from the NavSatFix message
        z = msg.altitude - self.alt_origin
